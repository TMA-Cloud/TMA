...
async function updateUserCustomDriveSettings(userId, enabled, path) {
  ...
  if (enabled && path) {
    ...
    try {
      ...
      const uploadFiles = await pool.query(
        'SELECT id, path, type FROM files WHERE user_id = $1 AND path IS NOT NULL',
        [userId]
      );
      ...
      if (uploadDirFiles.length > 0) {
        ...
        for (const file of uploadDirFiles) {
          if (file.type === 'file') {
            ...
            try {
              ...
              logger.warn({ filePath, fileId: file.id, error: error.message }, 'Could not delete upload file from disk');
        ...
        for (const folderPath of foldersToDelete) {
          ...
          try {
            ...
            logger.warn({ folderPath: fullFolderPath, error: error.message }, 'Could not delete upload folder from disk');
        ...
        if (successfullyDeletedFileIds.length > 0) {
          const deleteResult = await pool.query(
            'DELETE FROM files WHERE id = ANY($1::text[]) AND user_id = $2',
            [successfullyDeletedFileIds, userId]
          );
        ...
        if (failedDeletions.length > 0) {
          logger.warn({ userId, failedCount: failedDeletions.length, failedDeletions }, 'Some files could not be deleted from disk when enabling custom drive - database records preserved');
        }
    ...
    const allFiles = await pool.query(
      'SELECT id, path FROM files WHERE user_id = $1 AND path IS NOT NULL',
      [userId]
    );
