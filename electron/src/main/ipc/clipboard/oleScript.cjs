...
static byte[] GetFileContents(System.Runtime.InteropServices.ComTypes.IDataObject data,short cf,int index){try{var fmt=new FORMATETC();fmt.cfFormat=cf;fmt.dwAspect=DVASPECT.DVASPECT_CONTENT;fmt.lindex=index;fmt.ptd=IntPtr.Zero;fmt.tymed=TYMED.TYMED_HGLOBAL|TYMED.TYMED_ISTREAM|TYMED.TYMED_ISTORAGE;STGMEDIUM med;data.GetData(ref fmt,out med);try{if(med.tymed==TYMED.TYMED_HGLOBAL){IntPtr p=GlobalLock(med.unionmember);if(p==IntPtr.Zero)return null;try{int sz=GlobalSize(med.unionmember).ToInt32();if(sz<=0||sz>52428800)return null;var b=new byte[sz];Marshal.Copy(p,b,0,sz);return b;}finally{GlobalUnlock(med.unionmember);}}if(med.tymed==TYMED.TYMED_ISTREAM){var stm=(IStream)Marshal.GetObjectForIUnknown(med.unionmember);var stat=new System.Runtime.InteropServices.ComTypes.STATSTG();stm.Stat(out stat,0);long sz=stat.cbSize;if(sz<=0||sz>52428800)return null;var b=new byte[(int)sz];stm.Read(b,(int)sz,IntPtr.Zero);return b;}if(med.tymed==TYMED.TYMED_ISTORAGE){var iStg=(IStorage)Marshal.GetObjectForIUnknown(med.unionmember);ILockBytes iLb;IStorage iStg2;CreateILockBytesOnHGlobal(IntPtr.Zero,true,out iLb);StgCreateDocfileOnILockBytes(iLb,0x00001012,0,out iStg2);iStg.CopyTo(0,null,IntPtr.Zero,iStg2);iLb.Flush();iStg2.Commit(0);System.Runtime.InteropServices.ComTypes.STATSTG st;iLb.Stat(out st,1);long sz=st.cbSize;if(sz<=0||sz>52428800)return null;var b=new byte[(int)sz];int[] rd=new int[1];iLb.ReadAt(0,b,(int)sz,rd);Marshal.ReleaseComObject(iStg2);Marshal.ReleaseComObject(iLb);return b;}}finally{if(med.unionmember!=IntPtr.Zero)Marshal.Release(med.unionmember);}}catch{}return null;}
}'@
