...
export const SettingsField: React.FC<{
  ...
  return (
    <div className="flex flex-col gap-2 rounded-xl bg-white/60 dark:bg-gray-900/50 border border-slate-200/50 dark:border-slate-700/30 px-4 py-3 hover:border-blue-500/30 transition-all duration-200">
      {htmlFor ? (
        <label htmlFor={htmlFor} className="text-sm font-medium text-gray-900 dark:text-gray-100">
          {label}
        </label>
      ) : (
  ...
  return (
    <div className="flex justify-end gap-2">
      <button
        ...
        onClick={onCancel}
        disabled={disabled}
        className="px-4 py-2 text-sm font-medium rounded-lg border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 bg-[#ffffff] dark:bg-gray-800 hover:bg-[#f9f9fb] dark:hover:bg-gray-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
      >
        Cancel
      </button>
      <button
